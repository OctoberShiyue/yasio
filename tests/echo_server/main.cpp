...
void run_echo_server(const char* ip, u_short port, const char* protocol)
{
  ...
  server.schedule(std::chrono::milliseconds(100), [=](io_service& service) {
    /*std::string test_str = "最常见的定义是主题是文章中通过具体材料所表达的基本思想。这个定义由来已久，";
    std::string test_key = "测试用key";

    unsigned int plainLen = test_str.size() + 1;
    unsigned char* plain  = new unsigned char[plainLen];
    for (size_t i = 0; i < plainLen; i++)
    {
      plain[i] = 0;
    }
    std::memcpy(plain, test_str.c_str(), plainLen);

    unsigned int keyLen = test_key.size() + 1;
    unsigned char* key  = new unsigned char[keyLen];
    for (size_t i = 0; i < keyLen; i++)
    {
      key[i] = 0;
    }
    std::memcpy(key, test_key.c_str(), keyLen);

    AES aes(AESKeyLength::AES_256);

    unsigned char* plain2 = aes.EncryptECB(plain, 16 * sizeof(unsigned char), key);
    unsigned char* plain3 = aes.DecryptECB(plain2, 16 * sizeof(unsigned char), key);
    ...
    // 源文
    std::string source = "hello worlds!测试中文行不行！ 字符串-";
...
int main(int argc, char** argv)
{
  ...
  //// 设置公钥模数 n
  //rsa.set_modulus("a87441ebe810751e23ec1341315b0f3a87cb58f8e96b1ccaf03f5a6b7258c4dad563f2f533e04759a7e954c9a7e8ecd8f161a2830f5dc4e9dc66253aff85ac744940d3685873"
  //                "07b64ba00c7b02d4df6583057126d1960591078be9a1c212bf54571f1e9a30525010ca5e93329010545966c569d6b58b38502d55d4096bf8e26d");

  //// 设置私钥指数 d 这个私钥只能放在安全环境里使用
  //rsa.set_private_exp("53e8dd316a7e50287c524ae10d79c3632f633e6576b7f136b1678d5dba2eb7981df5547f89a0ad49de971eb1f85ed123db50fc0776af09b8481de56bb6fe5a011fdd8ef2"
  //                    "becca9fc1d480cb3ab5249c67ac87b2bac81b668ee837dc099d6ab1a1fe32cd8cbff192fca05e18b2e4291379bb1c167ef905b43548655be732ceec1");
  ...
  unsigned char* plain2 = aes.DecryptECB(aes.EncryptECB(plain, plainLen * 16, key), plainLen * 16, key);
  ...
  plain2 = nullptr;
  key    = nullptr;*/
  // printf("====[%s]\n", HexToBin("000102030405060708090a0b0c0d0e0f").c_str());
